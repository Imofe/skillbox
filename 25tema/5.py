...
class Car:
    ...
    Класс автомобиля
    Args:
        x, y (float) - начальные координаты авто
        direction (float) - направление (угол поворота)
    """
    def __init__(self, x: int, y: int, route: float):
        self.__x = x
        self.__y = y
        self.__route = route

    def __str__(self):
        return f'Координаты: X={round(self.__x, 2)} Y={round(self.__y, 2)}\nУгол {round(self.__route, 2)}'

    def move(self, distance):
        """
        Функция, которая говорит авто проехать дистанцию (distance)
        :param distance: дистанция, которую необходимо проехать
        """
        self.__x = self.__x + distance * math.cos(self.__route)
        self.__y = self.__y + distance * math.sin(self.__route)

    def change_direction(self, new_route):
        """
        функция, позволяющая автомобилю менять направление
        :param new_route: Новый угол поворота
        """
...
class Bus(Car):
    ...
    Args:
        ...
        route (float) - направление (угол поворота)
    """
