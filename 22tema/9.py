...
archive = zipfile.ZipFile('22tema/voyna-i-mir.zip', 'r')
archive.extractall()
archive.close()

book = open('22tema/voyna-i-mir.txt', 'r', encoding='utf8').read()
frequency = get_frequency(book)
for char_data in frequency:
    print(char_data[0] + " " + char_data[1])

# скрип работает, но у меня какая то проблема с работой с zip файлом, VScode не хочет его видеть :(